...
function AddProviderForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (missing.length) {
      await window.electronAPI?.alert(`Required: ${missing.map((f) => f.label).join(', ')}`);
      return;
    }
    setSaving(true);
    try {
...
export default function ModelsSettings() {
  ...
  const handleProviderRemove = async (keys: string[]) => {
    if (!(await window.electronAPI?.confirm(`Remove ${keys.join(', ')}?`))) {
      return;
    }
  ...
  return (
    <>
      ...
      {/* Favourite models */}
      <div className="mb-2">
        <SectionLabel className="mb-4">Favourite models</SectionLabel>
      </div>
      <div>
        <div className="mb-4">
          <div className="hint">
